...
export async function POST(request: Request) {
  try { const form = await request.formData(); const file = form.get("image"); const regionId = String(form.get("regionId") || "sample-seoul"); const sample = String(form.get("sample") || ""); const residenceType=String(form.get("residenceType")||""); const previousSummary=String(form.get("previousSummary")||"");
    ...
    const analysis = sample ? demoAnalysis(sample) : await aiAnalysis(imageDataUrl,{residenceType,previousSummary});
